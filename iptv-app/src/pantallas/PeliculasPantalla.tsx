...
import { View, Text, FlatList, StyleSheet, ActivityIndicator, Alert, Image, TouchableOpacity, TextInput, ScrollView, Modal } from 'react-native';
import iptvServicio, { VodStream, Category } from '../servicios/iptvServicio';
...
export const PeliculasPantalla = () => {
  ...
  const [peliculasFiltradas, setPeliculasFiltradas] = useState<VodStream[]>([]);
  ...
  useEffect(() => {
    filtrarPeliculas();
  }, [busqueda, peliculas, categoriaSeleccionada]);

  const cargarDatos = async () => {
    try {
      const [peliculasData, categoriasData] = await Promise.all([
  ...
  const filtrarPeliculas = () => {
    ...
    if (categoriaSeleccionada !== 'all') {
      filtradas = filtradas.filter((pelicula) =>
        pelicula.category_id === categoriaSeleccionada
      );
    }
    ...
    if (busqueda.trim() !== '') {
      filtradas = filtradas.filter((pelicula) =>
        pelicula.name.toLowerCase().includes(busqueda.toLowerCase())
      );
    }
  ...
  const getIcono = (nombre: string): string => {
    ...
    if (nombreLower.includes('western')) return '🤠';
  ...
  const renderCategoria = ({ item }: { item: Category }) => {
    ...
    const cantidadPeliculas = item.category_id === 'all' 
      ? peliculas.length 
      : peliculas.filter(p => p.category_id === item.category_id).length;
    ...
    return (
      ...
      >
        ...
        <View style={styles.categoriaItemRight}>
          <Text style={styles.cantidadText}>{cantidadPeliculas.toString()}</Text>
  ...
  const renderPelicula = ({ item }: { item: VodStream }) => (
    <TouchableOpacity
      style={styles.posterItem}
      onPress={() => verDetallesPelicula(item)}
    >
      {item.stream_icon ? (
        <Image
