...
import { SeriesInfo } from '../servicios/iptvServicio';
import tmdbServicio from '../servicios/tmdbServicio';
import { toggleFavorito, esFavorito, Favorito } from '../utils/favoritosStorage';

interface ModalDetallesSerieProps {
  visible: boolean;
  serie: SeriesInfo | null;
  onClose: () => void;
  onReproducir?: (serie: SeriesInfo) => void;
  usuarioId?: string;
  perfilId?: string;
}
...
export const ModalDetallesSerie: React.FC<ModalDetallesSerieProps> = ({
  visible,
  serie,
  onClose,
  onReproducir,
  ...
  const buscarDescripcion = async () => {
    ...
    try {
      ...
      const { pelicula: peliculaTMDB } = await tmdbServicio.buscarPeliculaConTrailer(serie.name);
  ...
  const reproducirSerie = () => {
    if (!serie || !onReproducir) return;
    onReproducir(serie);
    onClose();
  };
          ...
          <View style={styles.contentContainer}>
            ...
            <View style={styles.actionButtons}>
              <TouchableOpacity 
                ...
                onPress={reproducirSerie}
              >
                <Ionicons name="play" size={20} color="#FFF" />
                <Text style={styles.playButtonText}>Ver Serie</Text>
              </TouchableOpacity>
            ...
            <View style={styles.infoContainer}>
              {serie.genre && (
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoValue}>{serie.genre}</Text>
                </View>
              )}
              {serie.cast && (
                <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>Reparto:</Text>
                  <Text style={styles.infoValue} numberOfLines={2}>{serie.cast}</Text>
                </View>
              )}
            </View>
